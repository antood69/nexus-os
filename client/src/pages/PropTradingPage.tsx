...
import { Trophy, Plus, Trash2, AlertTriangle, TrendingUp, TrendingDown, DollarSign } from "lucide-react";
...
function DrawdownBar({ label, current, max }: { label: string; current: number; max: number }) {
  ...
  return (
    <div className="space-y-1">
      <div className="flex justify-between text-xs">
        ...
        <span className={isDanger ? "text-red-400" : isWarning ? "text-yellow-400" : "text-muted-foreground"}>
          {pct.toFixed(1)}% used
        </span>
      </div>
      <div className="w-full h-2 bg-secondary rounded-full overflow-hidden">
        <div
          className={`h-full rounded-full transition-all ${isDanger ? "bg-red-500" : isWarning ? "bg-yellow-500" : "bg-emerald-500"}`}
          style={{ width: `${pct}%` }}
        />
      </div>
    </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {accounts.map(acc => (
          <div key={acc.id} className="bg-card border border-border rounded-lg p-5 space-y-4">
            ...
            {acc.maxDrawdown && Math.abs(Math.min(0, acc.currentPnl)) / acc.maxDrawdown >= 0.7 && (
